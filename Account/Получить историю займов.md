...
|--------------|---------------|---------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------|
| currency     | нет           | string  | ***Валюта актива.***<br><br>- `USDC`,`USDT`,`BTC`,`ETH` и т.д.<br>- Только заглавными буквами                                                                                                                                                                                                                                                                                                                                       | -            |
| startTime    | нет           | integer | ***Временная метка начала выборки (в миллисекундах).***<br><br>- **`startTime` и `endTime` не передаются**:<br>по умолчанию возвращается 30 дней.<br>- **Передается только `startTime`**:<br>возвращается диапазон от времени начала + 30 дней.<br>- **Передается только `endTime`**:<br>возвращается диапазон от времени окончания - 30 дней.<br>- **Если передаются оба значения**:<br>время окончания - время начала <= 30 дней  | -            |
...
| currency                  | string  | ***Валюта актива.***<br><br>- `USDC`,`USDT`,`BTC`,`ETH` и т.д.                                         |
