...
Запрос списка своих кастодиальных sub-account для институционального клиента.
...
|uid |string |UID sub-account. |
|username |string |Имя пользователя sub-account. |
|memberType |integer |Тип аккаунта: <br>- `12`: Кастодиальный sub-account. |
|status |integer |Статус аккаунта<br>- `1`: Активен<br>- `2`: Вход запрещен<br>- `4`: Заморожен  |
|accountMode |integer |Режим аккаунта<br>- `1`: Классический аккаунт<br>- `3`: UTA-аккаунт  |
|remark |string |Примечание. |
|nextCursor |string |Метка для следующей страницы (используется в параметре запроса `nextCursor` для пагинации).<br>Возвращает `"0"`, если данных больше нет. |
