...
Запрос на создание нового API-ключа для недавно созданного sub-account.

>Информация:
>
>Используется только API-ключ master-account.
<!-- -->
>Важное замечание:
>
...
|---------------------------|---------------|----------|-----------------|------------|
|subuid |да |integer |***Идентификатор sub-account.*** |-   |
|note |нет |string |***Заметка для API-ключа.*** |-   |
|readOnly |да |integer |***Уровень доступа***<br><br>- `0`: Чтение и запись. <br>`1`: Только чтение. |-   |
|ips |нет |string |***Привязка к IP-адресам.***<br><br>- Если не передавать этот параметр или передать значение `"*"`, привязки к IP не будет.<br>- API-ключ без привязки к IP станет недействительным через 90 дней. <br>- API-ключ без привязки к IP станет недействительным через 7 дней после изменения пароля аккаунта. |-   |
...
|CopyTrading (данные объекта `permissions`)     |нет |array |***Copy Trading.***<br><br>- Пример и допустимые значения: [`"CopyTrading"`]<br>- **Устаревший параметр.** теперь используется [Как начать Copy Trading](</16.Как начать Copy Trading.md>) |-   |
...
|----------|----------|--------------------------------------------------------|
|id |string |Уникальный идентификатор (используется внутренне). |
|note |string |Заметка к API-ключу. |
|apiKey |string |Созданный API-ключ. |
|readOnly |integer |Уровень доступа<br>- `0`: Чтение и запись. <br>- `1`: Только чтение. |
|secret |string |Секретный ключ, связанный с API-ключом. <br> **Важно:** Секретный ключ нельзя получить через GET API. Сохраните его надежно. |
...
|Options |array |Разрешения для контрактов USDC (option и perpetual USDC). |
...
|CopyTrading |array |Разрешения для CopyTrading. **Устарел**, всегда возвращает `[]`. |
|BlockTrade |array |Разрешения для блочных сделок.<br>Неприменимо к sub-account, всегда возвращает `[]`. |
|NFT |array |**Устарел**, всегда возвращает `[]`. |
