...
Запрос информации об адресе депозита для sub-UID.

>Информация:
>
>- Используйте только API-ключ главного UID
>- Депозитный адрес кастодиального sub-UID получить невозможно
...
| chainType   | да            | string | Пожалуйста, используйте значение `chain` из [coin-info](</Asset/Получить информацию о монете.md>) | -            |
| subMemberId | да            | string | ***Идентификатор sub-UID.***                                                                      | -            |
...
| contractAddress   | string | ***Адрес контракта монеты.***<br><br>- Отображаются только последние 6 символов.<br>- Если адрес контракта отсутствует, отображается `""` |
