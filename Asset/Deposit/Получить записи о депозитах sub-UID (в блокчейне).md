...
| [status](</16.Определения значений в запросах и ответах.md#depositStatus>) | string  | ***Статус депозита.***                                                                                                                                                                 |
...
| batchReleaseLimit                                                          | string  | ***Лимит депозита для этой монеты в этой цепочке.***<br><br> `-1` означает отсутствие лимита                                                                                           |
| depositType                                                                | string  | ***Тип депозита.***<br><br>- `0`: нормальный депозит<br>- `10`: депозит достиг дневного лимита депозита<br>- `20`: ненормальный депозит                                                |
| fromAddress                                                                | string  | ***Адрес отправителя депозита.***<br><br>- Отображается только когда депозит поступает с блокчейн-источника и адрес отправителя уникален, в противном случае возвращает `""`           |
| taxDepositRecordsId                                                        | string  | ***Налоговый идентификатор.***<br><br>- Это поле используется в налоговых целях пользователями Bybit EU (Австрия)                                                                      |
| taxStatus                                                                  | integer | ***Это поле используется для налоговых целей пользователями Bybit EU (Австрия).***<br><br>- `0`: Отчетность не требуется<br>- `1`: Отчетность ожидается<br>- `2`: Отчетность завершена |
