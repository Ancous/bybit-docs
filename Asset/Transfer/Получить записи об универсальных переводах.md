...
> Важное замечание:
>
> - Поддерживаются как ключ API master-UID, так и ключ API sub-UID.
> - Для ключа API master-UID требуется разрешение «SubMemberTransfer».
> - Для ключа API sub-UID требуется разрешение «SubMemberTransferList».
<!-- -->
> Информация:  
>
