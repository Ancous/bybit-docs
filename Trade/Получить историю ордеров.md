...
>Информация:
>
>- Классический аккаунт `spot` может получить только статус закрытого ордера, а отмененные, отклоненные и
> деактивированные ордеры сохраняются до 7 дней.
...
|parentOrderLinkId|string|Указывает на связанный родительский ордер к которому прикреплены ордера тейк-профита и стоп-лосса. Поддерживается для фьючерсов и опционов.<br>- [Изменить ордер](</Trade/Изменить ордер.md>) тейк-профита или стоп-лосса не меняет `parentOrderLinkId`.<br>- Фьючерсы: использование [Установить стоп ордер](</Position/Установить стоп ордер.md>) для обновления прикреплённого TP/SL от исходного ордера не меняет `parentOrderLinkId`.<br>- Опционы: использование [Установить стоп ордер](</Position/Установить стоп ордер.md>) для обновления прикреплённого TP/SL от исходного ордера изменит `parentOrderLinkId`.<br>- Фьючерсы и Опционы: если TP/SL установлен через [Установить стоп ордер](</Position/Установить стоп ордер.md>) для позиции, у которой изначально не было прикреплённого TP/SL, `parentOrderLinkId` не имеет смысла   |
...
|cumExecValue    |string    |Общая стоимость исполненного ордера.<br>Классический аккаунт `spot` не поддерживается|
...
|smpGroup        |integer   |Идентификатор группы SMP. По умолчанию `0`, если нет группы|
