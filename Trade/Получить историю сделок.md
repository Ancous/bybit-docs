...
|orderLinkId                  |нет  |string      |***Пользовательский ID ордера***<br><br>Классический аккаунт не поддерживает этот параметр.              |-   |
...
|[execType](<../19.Определения значений в запросах и ответах.md#execType>)        |нет  |string     |***Тип исполнения.***<br><br>Классический спот не поддерживается.       |-   |
|limit                     |нет  |integer     |***Ограничение количества данных на странице.***<br><br>от `1` до `100`       |`50`   |
...
|[stopOrderType](<../19.Определения значений в запросах и ответах.md#stopOrderType>)   |string    |Тип стоп-ордера<br>Если ордер не является стоп-ордером, возвращается значение `UNKNOWN` или `""`.<br>Классический `spot` не поддерживается.|
...
|[execType](<../19.Определения значений в запросах и ответах.md#execType>)  |string      |Тип исполнения.<br>Классический `spot` не поддерживается.              |
|execValue  |string      |Сумма исполненного ордера.<br>Классический `spot` не поддерживается.                                            |
...
|feeRate  |string      |Ставка торговой комиссии.<br>Классический `spot` не поддерживается.                                             |
...
|markPrice  |string      |Цена маркировки символа на момент исполнения.<br>Классический `spot` не поддерживается.              |
