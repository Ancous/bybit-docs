...
>
>   - Bybit следит за активностью ваших API-запросов. Если число ордеров одного пользователя (учитывая master-account
>     и sub-account) за сутки (UTC 0 — UTC 24) превышает установленный предел, платформа оставляет за собой право
...
>   - При создании стоп-ордера вы получаете один идентификатор ордера.
>   - После срабатывания стоп-ордера, создаётся новый идентификатор ордера — то есть, ордер, который трейдится, уже
>    идёт с другим ID.
>- Единый аккаунт (UTA)
>   - При создании стоп-ордера вы получаете идентификатор ордера.
>   - После срабатывания стоп-ордера этот же идентификатор ордера сохраняется и не меняется — один и тот же идентификатор
