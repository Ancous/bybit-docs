...
|breakEvenPrice  | string|Цена безубыточности.<br><br>- Только для `linear` и `inverse`.<br>- Формула расчета: `breakeven_price` = (`entry_price` * `qty` - `realized_pnl`) / (`qty` - abs(`qty`) * max(`taker_fee_rate`, 0.00055))|
