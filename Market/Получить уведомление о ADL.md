...
>      `balance` > `adlTriggerThreshold` and `pnlRatio` < `insurancePnlRatio`  
>    - Условие остановки:  
...
>      `balance` > 0
>
>  - Избыточные потери по марже символа после его исключения из группового страхового пула (можно рассматривать как частный случай ADL по просадке капитала пула)
>
>    Для обеспечения безопасности пула команда риск-контроля может исключить символ из группового пула и временно создать для него новый независимый страховой пул.
>
>    - Условие срабатывания:  
>      `balance` ≤ 0
>    - Условие остановки:  
...
>
>    - Предположим, символы A, B и C используют общий пул с начальными 8-часовыми балансом 1 млн USDT.
