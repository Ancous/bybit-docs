...
|balance  |string       |Баланс фонда страхования.<br>Используется для определения срабатывания ADL<br>Для общего пула страхования поле `balance` будет обновляться по механизму T+1 и ежедневно в 00:00 UTC.  |
|maxBalance  |string       |**Устарело**<br>Всегда возвращает ""                                             |
|insurancePnlRatio  |string       |Пороговое значение коэффициента PnL для запуска ADL при просадке позиции<br>- ADL запускается, если коэффициент просадки позиции по данной торговой паре за последние 8 часов превышает это значение     |
|pnlRatio  |string       |Коэффициент просадки позиции по инструменту за последние 8 часов.<br>- Используется для определения того, должен ли быть запущен или остановлен механизм ADL    |
|adlTriggerThreshold  |string       |Порог срабатывания ADL по просадке позиции<br>- Данное условие действует только при остатке страхового фонда, превышающем это значение. В этом случае просадка более n% за 8 часов может запустить механизм ADL                                             |
|adlStopRatio  |string       |Порог остановки ADL по просадке позиции<br>- ADL отключаеться, когда коэффициент просадки позиции по инструменту за 8 часов опускается ниже этого значения.                                             |
