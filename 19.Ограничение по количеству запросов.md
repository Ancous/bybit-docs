...
>Инструкция по batch endpoints:
>
>Batch endpoint для ордеров, который включает операции создания, изменения и отмены, имеет собственное ограничение по
>скорости (rate limit) и не разделяет его с обычными одиночными запросами. Например, если ограничение на создание
>одиночных ордеров составляет 100 запросов в секунду, а ограничение на batch создание ордеров — тоже 100 запросов в
>секунду, то в сумме можно создать 200 линейных ордеров в секунду, используя оба этих эндпоинта.
>
>Когда категория = linear, spot или inverse,
>API для batch создания/изменения/отмены ордеров работает согласно текущей конфигурации частоты вызовов. Однако расход
>лимита учитывается в зависимости от фактического количества ордеров (расход лимита = количество запросов * количество
>ордеров в одном запросе). Конфигурация лимитов для разных бизнес-направлений независима друг от друга.
>
>Batch API позволяет отправлять от 1 до 10 ордеров в одном запросе. Например, если в batch запросе отправлено 5 ордеров,
>то расход лимита составит 5.
>
>Если часть последнего batch запроса в течение 1 секунды превышает лимит, то превышающая количество часть запросов не
>выполнится, а оставшаяся часть выполнится успешно. Например, если в течение секунды осталось 5 допускаемых вызовов,
>а отправлен batch запрос с 8 ордерами, то 5 первых ордеров будут выполнены успешно, а 6-8-й ордера вернут ошибку
>превышения лимита и не будут созданы.
...
<table class="iksweb">
		<tr>
			<td rowspan="2">HTTP метод</td>
			<td rowspan="2">Конечная точка</td>
			<td colspan="3"  align="center">Классический аккаунт</td>
			<td rowspan="2">Обновляемый</td>
		</tr>
		<tr>
			<td>inverse</td>
...
<table class="iksweb">
	<tbody>
		<tr>
			<td></td>
			<td colspan="3">Единый аккаунт</td>
		</tr>
		<tr>
			<td>Уровень</td>
			<td>Futures</td>
...
>эффективность и масштабируемость. Подробнее в объявлении.
