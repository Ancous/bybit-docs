...
|execFee | string | Исполненная торговая комиссия. Вы можете получить инструкцию по валюте комиссии spot [здесь](</19.Определения значений в запросах и ответах.md#SpotFeeCurrencyInstruction>). Классический аккаунт `spot` не поддерживается |
