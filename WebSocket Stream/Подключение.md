...
- [Пример подписки авторизации](#пример-подписки-авторизации)
- [Параметры подписки авторизации](#параметры-подписки-авторизации)
- [Пример ответа подписки авторизации](#пример-ответа-подписки-авторизации)
- [Параметры ответа подписки авторизации](#параметры-ответа-подписки-авторизации)
- [Лимиты IP](#лимиты-ip)
...
>Информация:
>
>- Если ваш аккаунт зарегистрирован на [www.bybit.eu](http://www.bybit.eu/), используйте `stream.bybit.eu` для
> доступа к основной сети.
...
<a id="пример-подписки-авторизации"></a>
...
<a id="параметры-подписки-авторизации"></a>

## Параметры подписки авторизации

| Параметр    | Обязательный  | Тип    | Комментарии                                                                                                                          | По умолчанию |
|-------------|---------------|--------|--------------------------------------------------------------------------------------------------------------------------------------|--------------|
| reqId       | нет           | string | ***Опциональное поле.***<br><br>- Используется для сопоставления ответа<br>- Если не передано, это поле не будет возвращено в ответе | -            |
| op          | да            | string | ***Тип операции.***                                                                                                                  | `auth`       |
| args        | да            | array  | ["api key", expires, "signature"]                                                                                                    | -            |

Это структура данных для Bybit WebSocket, переведенная на русский язык. Если нужны дополнительные разъяснения или примеры использования, дайте знать!|

<a id="пример-ответа-подписки-авторизации"></a>
...
<a id="параметры-ответа-подписки-авторизации"></a>

## Параметры ответа подписки авторизации

| Параметр | Тип     | Комментарии                                                                                                                                |
|----------|---------|--------------------------------------------------------------------------------------------------------------------------------------------|
| reqId    | string  | - Если передано в запросе, то возвращается в ответе<br>- Если не передано, то не возвращается в ответе или `""`                            |
| retCode  | integer | - `0`: аутентификация успешна<br>- `20001`: повторная аутентификация<br>- `10004`: недействительная подпись<br>- `10001`: ошибка параметра |
| retMsg   | string  | - `OK`<br>- `""`<br>- сообщение об ошибке                                                                                                  |
| op       | string  | ***Тип операции.***                                                                                                                        |
| connId   | string  | ***Идентификатор подключения.***<br><br>- Уникальный идентификатор для подключения                                                         |
...
>
>- Поддерживайте соединение активным, отправляя [пакет heartbeat](#как-отправить-пакет-Heartbeat)  
...
Независимо от `Perpetual`, `Futures`, `Option` или `Spot`, для одного публичного соединения длина массива "args"
