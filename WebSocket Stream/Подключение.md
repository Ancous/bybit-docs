...
|-----------------------------|------------|----------|---------------------------|------------|
|reqId |нет |string |Опциональное поле, используется для сопоставления ответа<br>- Если не передано, это поле не будет возвращено в ответе |-   |
...
|retMsg    |string    |`OK`, `""` или сообщение об ошибке |
|op        |string    |Тип операции     |
|connId    |string    |Идентификатор подключения, уникальный id для подключения |
...
- Mainnet - `wss://stream.bybit.com/v5` Testnet - `wss://stream-testnet.bybit.com/v5`

  - Публичный поток WebSocket

    Spot: `/public/spot`  
    USDT,USDC perpetual & USDT Futures: `/public/linear`  
    Inverse contract: `/public/inverse`  
    Spread trading: `/public/spread`  
    USDT/USDC Options: `/public/option`  
