...
- `Finished` Предварительная торговля завершена.
  - После аукциона, если предварительный контракт не переходит в фазу непрерывной торговли, он будет делистингован,
    и phase="Finished".
  - После непрерывной торговли, если предварительный контракт не конвертируется в официальный, он будет делистингован,
    и phase="Finished".
- `CallAuction` Фаза аукциона предварительной торговли.
  - Разрешено подавать только ордера с timeInForce=GTC и orderType=Limit.
  - TP/SL не поддерживаются; условные ордера не поддерживаются.
  - Нельзя модифицировать ордер на этом этапе.
  - Диапазон цены ордера: [preOpenPrice x 0.5, maxPrice].
- `CallAuctionNoCancel` Фаза аукциона без отмены предварительной торговли.
  - Разрешено подавать только ордера с timeInForce=GTC и orderType=Limit.
  - TP/SL не поддерживаются; условные ордера не поддерживаются.
  - Нельзя модифицировать и отменять ордер на этом этапе.
...
- `CrossMatching` Фаза кросс-матчинга.
  ...
  - Данные свечей публикуются с этого этапа.
- `ContinuousTrading` Фаза непрерывной торговли.
  - Нет ограничений на создание, изменение, отмену ордеров.
  - Данные ордербука и публичных сделок публикуются с этого этапа.
